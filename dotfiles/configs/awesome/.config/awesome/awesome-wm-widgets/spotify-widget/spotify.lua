...
local function worker(user_args)
	...
	local paused_icon = args.play_icon or "/usr/share/icons/Arc/actions/24/player_pause.png"
	local playing_icon = args.pause_icon or "/usr/share/icons/Arc/actions/24/player_play.png"
	...
	spotify_widget = wibox.widget({
		...
		set_status = function(self, is_playing)
			self:get_children_by_id("icon")[1]:set_image(is_playing and paused_icon or playing_icon)
	...
	local update_widget_text = function(widget, stdout, _, _, _)
		...
		if string.find(stdout, "ad-free") ~= nil then
			awful.spawn("sp kill", false, function()
				awful.spawn("spotify-launcher", false, function()
