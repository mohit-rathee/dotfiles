...
local function launch()
	...
	awful.prompt.run({
		...
		changed_callback = function()
			local textbox = spotify_shell.widget

			local text_width = select(1, textbox:get_preferred_size())

			-- icon + margins + prompt + padding
			w.width = math.max(250, text_width + 30)
			-- awful.placement.top(w, {
			-- 	parent = awful.screen.focused(),
			-- 	margins = { top = 40 },
			-- })
